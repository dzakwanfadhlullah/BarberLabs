...
function BookingContent() {
  ...
  const preselectedBarber = preBarber && preBarber !== 'any' ? getBarberById(preBarber) || null : null;
  const preselectedBranch = preselectedBarber
    ? getBranchById(preselectedBarber.branchId) || null
    : preBranch
      ? getBranchById(Number(preBranch)) || null
      : null;

  const [selectedBranch, setSelectedBranch] = useState<Branch | null>(preselectedBranch);
  const [selectedService, setSelectedService] = useState<Service | null>(
    preService ? getServiceById(preService) || services[0] : services[0]
  );
  const [selectedBarber, setSelectedBarber] = useState<Barber | null>(preselectedBarber);
  const [isAnyBarber, setIsAnyBarber] = useState(!preselectedBarber);
  ...
  const branchBarbers = useMemo(() => {
    return selectedBranch ? getBarbersByBranchId(selectedBranch.id) : [];
  }, [selectedBranch]);
  ...
  const handleBranchSelect = (branch: Branch) => {
    ...
    setErrors(prev => ({ ...prev, branch: '', barber: '', date: '', time: '' }));
  };
  ...
  const closedDays = useMemo(() => {
    const shopClosedDays = getClosedDaysFromHours();

    if (!selectedBranch) return shopClosedDays;

    const availableDays = isAnyBarber
      ? Array.from(new Set(branchBarbers.flatMap(barber => barber.availableDays)))
      : selectedBarber?.availableDays || [];

    return DAY_INDEXES.filter((day) => shopClosedDays.includes(day) || !availableDays.includes(day));
  }, [branchBarbers, isAnyBarber, selectedBarber, selectedBranch]);
  ...
  const timeSlots = useMemo(() => {
    ...
    const hasCapsterAvailable = isAnyBarber
      ? branchBarbers.some(barber => barber.availableDays.includes(dayIndex))
      : selectedBarber?.availableDays.includes(dayIndex);

    if (!hasCapsterAvailable) return [];

    return generateTimeSlots(dayHours.open, dayHours.close);
  }, [branchBarbers, isAnyBarber, selectedBarber, selectedBranch, selectedDate]);
  ...
  const validate = (): boolean => {
    ...
    if (!selectedBranch) e.branch = 'Choose a branch.';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr_320px] gap-8 lg:gap-12 xl:gap-16">
        ...
        >
          ...
          <p className="mt-4 text-[16px] leading-[1.4] text-[var(--color-text-secondary)] max-w-[260px]">
            Choose your branch, service, capster, and time.
          </p>
        </motion.div>
        ...
        >
          <div>
            <SectionLabel number="02" title="Branch" className="mb-4" />
            <BookingBranchSelector branches={branches} selected={selectedBranch} onSelect={handleBranchSelect} />
            {errors.branch && <p className="mt-2 text-[13px] text-[var(--color-error)]">{errors.branch}</p>}
          </div>

          <div>
            <SectionLabel number="03" title="Service" className="mb-4" />
          ...
          <div>
            <SectionLabel number="04" title="Capster" className="mb-4" />
          ...
          <div>
            <SectionLabel number="05" title="Date" className="mb-4" />
          ...
          <div>
            <SectionLabel number="06" title="Time" className="mb-4" />
            {selectedDate ? (
              <>
                {timeSlots.length > 0 && (
          ...
          <div>
            <SectionLabel number="07" title="Contact" className="mb-4" />
          ...
          <div className="lg:hidden">
            <div className="border border-[var(--color-border)] p-4 mb-4">
              ...
              <BookingSummary branch={selectedBranch} service={selectedService} barber={selectedBarber} isAnyBarber={isAnyBarber} date={selectedDate} time={selectedTime} />
            </div>
        ...
        >
          <div className="sticky top-[90px]">
            <div className="border-l border-[var(--color-border)] pl-8">
              <SectionLabel number="08" title="Your Booking" className="mb-6" />
              <BookingSummary branch={selectedBranch} service={selectedService} barber={selectedBarber} isAnyBarber={isAnyBarber} date={selectedDate} time={selectedTime} />
              <div className="mt-6">
                <PrimaryButton onClick={handleConfirm} fullWidth icon={<MessageCircle className="w-4 h-4" />}>
                  Confirm via WhatsApp
            ...
            <div className="border-l border-[var(--color-border)] pl-8 mt-10">
              <SectionLabel number="09" title="Booking Notes" className="mb-4" />
