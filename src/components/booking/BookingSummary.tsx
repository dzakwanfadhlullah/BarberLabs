...
export function BookingSummary({ service, branch, barber, isAnyBarber, date, time }: BookingSummaryProps) {
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-3">
        <MapPin className="w-4 h-4 mt-0.5 text-[var(--color-text-muted)]" />
        <div>
          <p className="text-[12px] text-[var(--color-text-muted)] uppercase tracking-wide">Branch</p>
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="text-[12px] text-[var(--color-text-muted)] uppercase tracking-wide">Capster</p>
          <p className="text-[15px] font-semibold text-black mt-0.5">
            {isAnyBarber ? 'Any Capster' : barber?.name || '-'}
          </p>
          {isAnyBarber && (
